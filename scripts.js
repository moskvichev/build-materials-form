...
function calculateOrder() {
  ...
  let result = calculateOrderInternal(
    parseInt(length.value),
    parseInt(height.value),
    isNeedAssembly.checked,
    parseInt(material.options[material.selectedIndex].value),
  );
...
function sendEmailInternal(To) {
  ...
  let Body =
    name.value +
    ', заказ №1 сформирован. В ближайшее время наш специалист свяжется с вами по телефону ' +
    phone.value;
...
function localization() {
  ...
  document.getElementsByClassName('form__input_measurement')[0].innerHTML = decliner(
    parseInt(length),
    ' метр',
    ' метра',
    ' метров',
  );
  document.getElementsByClassName('form__input_measurement')[1].innerHTML = decliner(
    parseInt(height),
    ' метр',
    ' метра',
    ' метров',
...
function textInputSecondPage() {
  ...
  document.getElementById('metr_length').innerHTML = decliner(
    parseInt(metrLength),
    ' метр',
    ' метра',
    ' метров',
  ...
  document.getElementById('metr_height').innerHTML = decliner(
    parseInt(metrHeight),
    ' метр',
    ' метра',
    ' метров',
